...
Twinkle.speedy.initDialog = function twinklespeedyInitDialog(callbackfunc) {
	...
	dialog.addFooterLink(wgULS('快速删除方针', '快速刪除方針'), 'CSD');
	dialog.addFooterLink(wgULS('常见错误', '常見錯誤'), 'Project:管理员错误自查表/快速删除');
	dialog.addFooterLink(wgULS('速删设置', '速刪設定'), 'TW/PREF#speedy');
	dialog.addFooterLink(wgULS('Twinkle帮助', 'Twinkle說明'), 'TW/DOC#speedy');
	...
	sysop: {
		...
		deleteTalk: function(params) {
			...
			if (params.deleteTalkPage &&
				...
				talkpage.setEditSummary('[[CSD#G15|G15]]: 孤立页面: 已删除页面“' + Morebits.pageNameNorm + '”的讨论页');
		...
		deleteRedirectsMain: function(apiobj) {
			...
			$snapshot.each(function(key, value) {
				...
				page.setEditSummary('[[CSD#G15|G15]]: 孤立页面: 重定向到已删除页面“' + Morebits.pageNameNorm + '”');
	...
	user: {
		main: function(pageobj) {
			...
			if (params.normalizeds.length > 1) {
				...
				$.each(params.normalizeds, function(index, norm) {
					if (norm !== 'db') {
						editsummary += '[[CSD#' + norm.toUpperCase() + '|CSD ' + norm.toUpperCase() + ']]、';
					}
				});
				editsummary = editsummary.substr(0, editsummary.length - 1); // remove trailing comma
				editsummary += '）';
			} else if (params.normalizeds[0] === 'db') {
				editsummary = wgULS('请求[[CSD|快速删除]]：', '請求[[CSD|快速刪除]]：') + params.templateParams[0]['1'];
			} else {
				editsummary = wgULS('请求快速删除', '請求快速刪除') + '（[[CSD#' + params.normalizeds[0].toUpperCase() + '|CSD ' + params.normalizeds[0].toUpperCase() + ']]）';
			}
		...
		addToLog: function(params, initialContrib) {
			...
			usl.initialText =
				'这是该用户使用[[TW|Twinkle]]的速删模块做出的[[CSD|快速删除]]提名列表。\n\n' +
			...
			if (params.fromDI) {
				if (params.normalized === 'f3 f4') {
					appendText += '图版[[CSD#F3|CSD F3]]+[[CSD#F4|CSD F4]]（{{tl|no source no license/auto}}）';
				} else {
					appendText += '图版[[CSD#' + params.normalized.toUpperCase() + '|CSD ' + params.normalized.toUpperCase() + ']]（{{tl|' + params.templatename + '}}）';
				}
			} else {
				if (params.normalizeds.length > 1) {
					appendText += '多个理由（';
					$.each(params.normalizeds, function(index, norm) {
						appendText += '[[CSD#' + norm.toUpperCase() + '|' + norm.toUpperCase() + ']]、';
					});
					...
					appendText += '[[CSD#' + params.normalizeds[0].toUpperCase() + '|CSD ' + params.normalizeds[0].toUpperCase() + ']]';
				}
			}
				...
				if (form['csd.g5_1']) {
					...
					if (deldisc) {
						if (!/^(Project|PROJ|红砖维基|红砖維基):/i.test(deldisc)) {
							alert(wgULS('CSD G5：您提供的讨论页名必须以“红砖维基:”开头。', 'CSD G5：您提供的討論頁名必須以「红砖维基:」開頭。'));
							parameters = null;
							return false;
						}
